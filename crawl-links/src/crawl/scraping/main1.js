    ...
    if (existsSync(`src/history/${projectUid}`)) {
      ...
      if (temp.length == pageEachScrapeWave || i == 0 || i == UNIQUE_CRAWLABLE_LINKS.length - 1) {
        ...
        await Promise.all(temp).then((fetchData) => {
          const dataLength = fetchData.length
          for (let index = 0; index < dataLength; index++) {
            ...
            if (eachData) {
              ...
              let quantity_of_crawled_link = 0;
                ...
                const finished_s = (await measureTime(async() => {
                ...
                avgSpeed = Math.max(finished_s, avgSpeed);
              })();
            }
          }
            ...
            const avg = (avgSpeed +
                        ...
                        (UNIQUE_CRAWLABLE_LINKS.length + dataLength)
